    ...
    <form onSubmit={handleSubmit(onNext)} className="space-y-4">
      ...
      <div>
        ...
        {errors.residentNumber && (
          <p className="text-xs text-red-500 mt-1">{errors.residentNumber.message}</p>
        )}
        <p className="text-xs text-gray-400 mt-1">주민등록초본 자동 발급에만 사용되며 저장되지 않습니다.</p>
      </div>

      <div>
        <label className="block text-sm font-medium mb-1">주소 (전입 주소)</label>
        <input
          className="input-field"
